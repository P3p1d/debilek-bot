...
class Economy(commands.Cog):
	...
	async def buy(self,ctx,bizid:int=0,amount_to_buy:int=1):
		...
		if "last_check" not in a:
			self.d[guild].update_one({"name":str(ctx.message.author)},{"$set":{"amount":a["amount"]-float(biz["price"]*amount_to_buy),"pers":upers,"last_check":datetime.datetime.utcnow()}})
		else:
			self.d[guild].update_one({"name":str(ctx.message.author)},{"$set":{"amount":a["amount"]-float(biz["price"]*amount_to_buy),"pers":upers}})
